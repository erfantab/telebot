...
def send_welcome(message):
    ...
    # فقط اگه عضو بود
    welcome_msg = "قراره تو عکس بفرستی، ما ذوق کنیم، بقیه رأی بدن، و یه نفر قهرمان شه! حالا نوبت توئه 🎯👑.فقط حواست باشه که یه یوزرنیم زیرش بزاری اگه برنده شدیم باهات ارتباط بگیریم."
...
def handle_photo(message):
    ...
    first_name = (user.first_name or "ندارد").replace("_", "\\_")
    last_name = (user.last_name or "ندارد").replace("_", "\\_")
    ...
    print("Forwarding photo and user info...")  # لاگ برای بررسی
    for admin_id in ADMIN_IDS:
        try:
            # فوروارد عکس
            bot.forward_message(admin_id, message.chat.id, message.message_id)

            # ارسال پیام اطلاعات کاربر
            bot.send_message(admin_id, user_info, parse_mode="Markdown")
            print(f"Info sent to admin {admin_id}")
        except Exception as e:
            print(f"Error sending info to admin {admin_id}: {e}")
